# Parity refresh report - 2026-05-27
...
- pgschema [#414](https://github.com/pgplex/pgschema/issues/414): creating a
  ...
  - evidence: `tests/integration/mixed-objects.test.ts` roundtrips `ADD COLUMN`
    plus view creation, and pg-delta's logical sort keeps table changes ahead
    of view creation
...
- pgschema [#418](https://github.com/pgplex/pgschema/issues/418):
  ...
  - classified as `not_parity`; this is specific to pgschema's online-DDL
    rewrite and pg-delta does not synthesize `CONCURRENTLY`
...
- pgschema [#422](https://github.com/pgplex/pgschema/issues/422): quoted
  ...
  - both are tied to pgschema's dump -> temp-schema -> plan roundtrip path and
    do not map cleanly onto pg-delta's catalog-diff workflow
...
- There is no parity-state delta versus the 2026-05-26 refresh.
...
- New pg-toolbelt issue
  [#263](https://github.com/supabase/pg-toolbelt/issues/263) is adjacent
  dependency-chain work (`ALTER COLUMN TYPE` / `DROP FUNCTION` with policy and
  view dependents), but it does not supersede the benchmarked table-constraint
  gap (#412) or the existing parity trackers for pgschema #404 and #366.
