# Parity refresh report - 2026-06-16
...
There is no resolved-issue parity-state delta versus the 2026-06-15 refresh.
...
## 2) Upstream movement since 2026-06-15
...
- the newest open pgschema issues are still
  ...
  [#473](https://github.com/pgplex/pgschema/issues/473)
- closed upstream items that still matter to the benchmark remain unchanged:
  [#412](https://github.com/pgplex/pgschema/issues/412),
  [#439](https://github.com/pgplex/pgschema/issues/439), and
  [#444](https://github.com/pgplex/pgschema/issues/444) are still closed
- adjacent pg-toolbelt dependency-chain work
  [#263](https://github.com/supabase/pg-toolbelt/issues/263),
  [#285](https://github.com/supabase/pg-toolbelt/pull/285), and
  [#291](https://github.com/supabase/pg-toolbelt/pull/291) is still context
  only, not an exact tracker for historical gaps #439 or #444
...
- pgschema [#471](https://github.com/pgplex/pgschema/issues/471): partitioned
  ...
  extraction and diff path; the focused June 16 probe still emits
...
- `bun test packages/pg-delta/src/core/objects/table/table.diff.test.ts`
  ...
  with **25 passing tests**
- a focused June 16 Bun probe exercised both the unresolved table-constraint
  `NULLS NOT DISTINCT` case and the covered partitioned-table RLS case. Current
  pg-delta returned:
  ...
  That keeps benchmark 020 open while preserving the **covered** verdict for
  pgschema #471.
...
- `DRY_RUN=true python3 scripts/compare_issues.py` and
  ...
  the current scripts still filter on upstream `Bug` / `Feature` labels while
  the relevant live pgschema issues are unlabeled
