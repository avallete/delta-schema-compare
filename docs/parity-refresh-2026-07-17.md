# Parity refresh report - 2026-07-17

This report records the 2026-07-17 latest-state parity check between:
...
[`docs/parity-refresh-2026-07-16.md`](./parity-refresh-2026-07-16.md).
...
## 2) Upstream delta on 2026-07-17

This refresh again found **no new pgschema main movement** and **no pg-delta
engine delta** relative to the checked-in benchmark baseline:
...
- the only new open-item delta versus the 2026-07-16 sweep is
  [#513](https://github.com/pgplex/pgschema/issues/513)
  (`Full migration handling workflow?`), which remains **not parity work** for
  pg-delta because it is workflow / migration-history orchestration rather than
  a live-catalog diff or current planner gap
- direct issue-page checks confirm the tracked and draft-only resolved items
  keep the same parity state:
...
- the latest pg-toolbelt tracker state is still non-duplicative and unresolved:
  ...
  - adjacent work in [#263](https://github.com/supabase/pg-toolbelt/issues/263),
    ...
    [#310](https://github.com/supabase/pg-toolbelt/pull/310) still does not
    exactly cover benchmarks **020** through **023** or the older draft-only
    items [#439](https://github.com/pgplex/pgschema/issues/439) and
    [#444](https://github.com/pgplex/pgschema/issues/444)
  - new adjacent issue
    [#334](https://github.com/supabase/pg-toolbelt/issues/334) and merged PR
    [#335](https://github.com/supabase/pg-toolbelt/pull/335) cover broader
    dependency-ordering and plan/apply fixes, but they still do not exactly
    match benchmarks **020** through **023** or the older draft-only items
    **#439** and **#444**
- the July 17 duplicate search for the four active benchmarks and the two
  draft-only uncovered items again returned only adjacent or broader work, not
  any exact tracker we should treat as a replacement benchmark or duplicate

## 3) What changed in this refresh

Because the latest-state sweep again found no benchmark-matrix change, this
refresh keeps the benchmark matrix and checked-in submodule pointers unchanged
and records the result as another dated no-delta report:

- add this report as the 2026-07-17 latest-state sweep
- annotate `benchmark/README.md` so the source-of-truth matrix points to this
  follow-up, records the new non-parity open issue [#513](https://github.com/pgplex/pgschema/issues/513),
  and notes that adjacent pg-toolbelt issue / PR activity still does not remove
  any active or draft-only parity gaps
...
- add [#513](https://github.com/pgplex/pgschema/issues/513) to
  `benchmark/review-memory.json` as an open `not_parity` item so future sweeps
  do not keep re-screening it as new work
...
- live GitHub issue-page checks for pgschema **#49**, **#52**, **#84**,
  ...
  **#499**, **#501**, **#505**, **#506**, **#508**, **#509**, and **#513**
- live GitHub issue / PR checks for pg-toolbelt **#218**, **#219**, **#263**,
  **#285**, **#291**, **#299**, **#308**, **#310**, **#329**, **#334**, and
  **#335**
- GitHub search checks for exact-duplicate pg-toolbelt trackers for benchmarks
  **020**, **021**, **022**, **023**, and the older draft-only unresolved items
  **#439** and **#444**
- direct git diff of live pg-toolbelt `main` versus the checked-in baseline,
  confirming the same CI-only `.github/**` delta as the 2026-07-16 sweep
- targeted pg-delta unit tests:
  - after submodule initialization, `bun install --frozen-lockfile` restores
