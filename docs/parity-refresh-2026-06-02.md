# Parity refresh report - 2026-06-02
...
- pgschema [#427](https://github.com/pgplex/pgschema/issues/427):
  ...
  - policy extraction preserves the stored expression via
    `pg_get_expr(...)` in
...
No newly closed parity-relevant pgschema issues appeared since the
2026-06-01 refresh. The current screened closed-item state remains:
...
- The resolved benchmark set is unchanged versus the 2026-06-01 refresh.
...
- The notable delta from the previous refresh is that pgschema #427 is now
  reclassified from draft-only uncovered to covered, because current pg-delta
  already preserves schema-qualified policy expressions and exercises them in
  the live RLS integration suite.
- There is still no exact matching pg-toolbelt issue or PR for pgschema #412,
  #420, #439, or #444.
- No new parity-relevant pgschema issues, issue updates, or merged PRs landed
  since 2026-06-01; the only status delta in this refresh comes from
  re-validating the current pg-delta side.
- The older draft text in `docs/parity-issue-drafts-2026-05-23.md` is now
  historical only for #427 and should not be treated as a live uncovered
  candidate.
