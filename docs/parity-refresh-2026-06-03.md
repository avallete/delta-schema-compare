# Parity refresh report - 2026-06-03
...
- pgschema [#420](https://github.com/pgplex/pgschema/issues/420):
  ...
  - column extraction uses `format_type(a.atttypid, a.atttypmod)` in
    `src/core/objects/table/table.model.ts`
  - type diffs compare `data_type_str` in
    `src/core/objects/table/table.diff.ts`
  - table create / alter SQL serialize `data_type_str` verbatim in
    `src/core/objects/table/changes/table.create.ts` and
    `src/core/objects/table/changes/table.alter.ts`
  - no matching pg-toolbelt issue or PR was found, and the older draft text in
...
- pgschema [#427](https://github.com/pgplex/pgschema/issues/427):
  ...
  - current pg-delta already roundtrips a policy that calls a
    schema-qualified function in
    `tests/integration/rls-operations.test.ts`
  - `tests/integration/policy-dependencies.test.ts` also covers the related
    policy / function dependency ordering path
  - policy extraction preserves the stored expression via `pg_get_expr(...)` in
...
No new parity-relevant pgschema merged PRs landed since the 2026-06-02
refresh. The current screened closed-item state is:
...
- pgschema [#446](https://github.com/pgplex/pgschema/issues/446): explicit
  ...
  - `constraint_type` includes both `p` and `u` in
    `src/core/objects/table/table.model.ts`
  - constraint diffs compare extracted constraints directly in
    `src/core/objects/table/table.diff.ts`
  - no matching pg-toolbelt issue or PR was found

## 4) Notes

- The resolved benchmark set is unchanged versus the 2026-06-02 refresh.
...
- The notable deltas from the previous refresh are:
  - pgschema #420 is reclassified from draft-only `not_covered` to `covered`
  - pgschema #446 is reclassified from `not_parity` to `covered`
  - new open issue #447 is screened as `not_parity`
- There is still no exact matching pg-toolbelt issue or PR for pgschema #412,
  #439, or #444.
- No new parity-relevant pgschema fixes landed since 2026-06-02; the June 3
  changes come from re-validating the current pg-delta side and screening the
  new `.pgschemaignore` issue.
