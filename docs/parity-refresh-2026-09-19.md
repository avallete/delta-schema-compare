# Parity refresh report - 2026-09-19

This report records the 2026-09-19 latest-state parity check between:
...
This refresh finds **no checked-in/live submodule code delta** versus
[`docs/parity-refresh-2026-09-18.md`](./parity-refresh-2026-09-18.md):
...
## 2) Open / resolved pgschema delta on 2026-09-19

There is still real upstream activity since the 2026-09-18 refresh even
though the checked-in/live code heads did not move:

- new closed pgschema issue
  [#609](https://github.com/pgplex/pgschema/issues/609)
  (`plan intermittently fails with "no version found matching 17.5.0"`)
- new open pgschema PR
  [#610](https://github.com/pgplex/pgschema/pull/610) as the upstream fix path
  for issue [#606](https://github.com/pgplex/pgschema/issues/606)
- open pgschema PR
  [#608](https://github.com/pgplex/pgschema/pull/608) remains the upstream fix
  path for issue [#607](https://github.com/pgplex/pgschema/issues/607)
...
- **#606** remains **covered** in current pg-delta:
  ...
  - `repos/pg-toolbelt/packages/pg-delta/src/plan/rules/constraints.ts`
    ...
    `ADD CONSTRAINT`
  - open pgschema PR [#610](https://github.com/pgplex/pgschema/pull/610)
    confirms the upstream bug is about pgschema's own partitioned-table key
    reordering, not a missing pg-delta behavior
- **#607** remains **not parity work** for pg-delta:
  - it is still the same temp comparison schema / `search_path` resolution
    class as previously screened pgschema issue
...
- **#609** is **not parity work** for pg-delta:
  - the failure is specific to pgschema's embedded desired-state PostgreSQL
    startup path in offline / air-gapped environments
  - pg-delta diffs live source and target catalogs directly and does not need
    to download or start an embedded comparison database
...
- refresh `benchmark/README.md` to the 2026-09-19 latest-state snapshot
- add fresh 2026-09-19 refresh notes to benchmarks
...
- refresh `benchmark/review-memory.json` review timestamps for the rechecked
  open watch-list items and active benchmark issues, and add resolved issue
  **#609**
- add this report as the 2026-09-19 latest-state sweep
...
- GitHub CLI updated-state queries for both upstream repos:
  - `gh issue list -R pgplex/pgschema --state open --limit 40 --json number,title,updatedAt,labels,url`
  ...
  - `gh issue list -R supabase/pg-toolbelt --state open --limit 50 --json number,title,updatedAt,labels,url`
...
- focused issue / PR inspection for the fresh upstream activity:
  - `gh issue view 609 -R pgplex/pgschema --json number,title,body,updatedAt,url,state,closedAt`
  - `gh pr view 610 -R pgplex/pgschema --json number,title,body,updatedAt,url,state,isDraft,mergeStateStatus,commits`
...
- no new pg-delta runtime probes were rerun in this pod because Bun and Docker
  are unavailable here; the verdicts rely on source inspection plus already
