# Parity refresh report - 2026-06-15
...
There is no resolved-issue parity-state delta versus the 2026-06-14 refresh.
...
## 2) Upstream movement since 2026-06-14
...
- pgschema [#471](https://github.com/pgplex/pgschema/issues/471): partitioned
  ...
  extraction and diff path; a focused June 15 probe against a partitioned
  parent still emits `ALTER TABLE ... ENABLE ROW LEVEL SECURITY`
...
- a focused June 15 Bun probe constructed two otherwise-identical table
...
- the same probe also exercised a partitioned parent table with only
  `row_security` toggled and the current diff code returned:
  ...
  Combined with table extraction covering relkind `('r', 'p')`, that keeps the
  parity verdict for pgschema #471 at **covered**.
