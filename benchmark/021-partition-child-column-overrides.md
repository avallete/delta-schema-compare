...
and the new pgschema activity does not change this benchmark:
...
- open pgschema PR [#610](https://github.com/pgplex/pgschema/pull/610) is the
  ...
  which current pg-delta already covers and which is separate from child-local
  partition overrides
...
- `repos/pg-toolbelt/packages/pg-delta/src/plan/rules/tables.ts` still emits
  `CREATE TABLE ... PARTITION OF ... ${bound}` with no typed child
  column-element list for local `DEFAULT` / `NOT NULL` overrides
- merged PR [#480](https://github.com/supabase/pg-toolbelt/pull/480) fixes an
  adjacent partition-index drop-root cycle, but it still does not add
  child-local partition override extraction or rendering; umbrella issue
...
Today's upstream code movement is still release-only rather than gap-closing
for this benchmark:
...
this benchmark's active uncovered slice. The live pg-delta delta is narrower
but still not gap-closing:
...
- merged PR [#470](https://github.com/supabase/pg-toolbelt/pull/470) /
  alpha.51 rebuilds replace-path dependents, publication membership, and
  attached partition indexes, but it still does not extract or render
  child-local partition overrides.
...
[#332](https://github.com/supabase/pg-toolbelt/issues/332) and unrelated open
issue [#451](https://github.com/supabase/pg-toolbelt/issues/451) remain the
only open tracker context, while merged PR
[#470](https://github.com/supabase/pg-toolbelt/pull/470) is adjacent
retrospective context. Benchmark 021 therefore remains **behaviorally
uncovered** with **umbrella-thread tracker context only**.
...
outside this partition-child benchmark. The active partition-child override gap
is unchanged:
...
`.pgschemaignore` view-filtering report and remains outside this partition-child
benchmark. The active partition-child override gap is therefore unchanged:
...
- open PR [#470](https://github.com/supabase/pg-toolbelt/pull/470) remains
  adjacent only, and open PRs
  [#471](https://github.com/supabase/pg-toolbelt/pull/471) /
  [#472](https://github.com/supabase/pg-toolbelt/pull/472) remain unrelated.
